...
{   
    ...
    pr_info("osfs_create: Creating file '%.*s' in directory inode %lu\n",
            (int)dentry -> d_name.len, dentry -> d_name.name, dir -> i_ino);

    // Step2: Validate the file name length //檔案名稱驗證
    if(dentry -> d_name.len > MAX_FILENAME_LEN){
        pr_err("osfs_create: File name '%.*s' is too long\n", (int)dentry -> d_name.len, dentry -> d_name.name);
        return -ENAMETOOLONG;
    }

    // Step3: Allocate and initialize VFS & osfs inode //分配新的索引節點
    ...
    // Step 5: Update the parent directory's metadata //更新父目錄元數據
    parent_inode -> i_size += sizeof(struct osfs_dir_entry);
    mark_inode_dirty(dir); //標記為修改
    
    // Step 6: Bind the inode to the VFS dentry //綁定索引節點與dentry
