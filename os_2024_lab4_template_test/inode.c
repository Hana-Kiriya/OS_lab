 ...
 */
int osfs_alloc_data_block(struct osfs_sb_info *sb_info, uint32_t *start_block, uint32_t requested_length)
{
    uint32_t i, j;
    ...
    for (i = 0; i < sb_info->block_count; i++) {
        ...
        if(found_length == requested_length){
            uint32_t start = i - requested_length + 1;
            for(j = start; j <= i; j++){
                set_bit(j, sb_info->block_bitmap);
            }
            
            sb_info->nr_free_blocks -= requested_length;
            *start_block = start;
            return 0;
        }
    }
